...
<template>
  <div class="json-lab">
    ...
    <section class="lab-section">
      ...
      <!-- Activity 13 completed with @click, v-if and v-else. -->
...
const authorTitle = (author) =>
  isGeorgeOrwell(author)
    ? "George Orwell is highlighted with attribute, class and style binding"
    : `${author.name} is not the highlighted author`

const authorStyle = (author) => ({
  border: isGeorgeOrwell(author) ? "2px solid #2f855a" : "1px solid #ddd",
  fontWeight: isGeorgeOrwell(author) ? "700" : "400",
})
</script>
